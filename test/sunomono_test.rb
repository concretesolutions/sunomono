...
describe Sunomono do
  ...
  context 'Using I18n to translate files' do
    it 'Generate an OS independent feature using I18n to portuguese language' do
      ...
      expect(File.readlines("#{@project_name}/features/#{feature_name}.feature")).to include("# language: pt\n", "Funcionalidade: #{feature_name.capitalize} \n", "\n", "  Contexto:\n", "    # Insira os passos\n", "    \n", "  Cenário: Primeiro Cenario\n", "    # Insira os passos\n")
      expect(File.readlines("#{@project_name}/features/step_definitions/#{feature_name}_steps.rb")).to include("######### DADO #########\n", "\n", "######### QUANDO #########\n", "\n", "######### ENTãO #########")
    end
    ...
    it 'Generate an OS independent feature using english language' do
      ...
      expect(File.readlines("#{@project_name}/features/#{feature_name}.feature")).to include("# language: en\n", "Feature: #{feature_name.capitalize} \n", "\n", "  Background:\n", "    # Insert steps\n", "    \n", "  Scenario: First Scenario\n", "    # Insert steps\n")
      expect(File.readlines("#{@project_name}/features/step_definitions/#{feature_name}_steps.rb")).to include("######### GIVEN #########\n", "\n", "######### WHEN #########\n", "\n", "######### THEN #########")
    end
  end
  ...
  context 'Generates an OS independent step' do
    it 'Command cant generates .feature and screens files' do
      ...
      expect(Dir.entries("#{@project_name}/features")).not_to include("#{feature_name}.feature")
      expect(Dir.entries("#{@project_name}/features/android")).not_to include("#{feature_name}_screen.rb")
      expect(Dir.entries("#{@project_name}/features/ios")).not_to include("#{feature_name}_screen.rb")
    end
  end
